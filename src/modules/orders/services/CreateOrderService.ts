...
class CreateOrderService {
  public async execute({customer_id, products}: IRequest): Promise<Order>{
    ...
    const quantityAvaible = products.filter(
      product =>
        existsProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity,
    );

    if (quantityAvaible.length) {
      throw new AppError(`Then quantity ${quantityAvaible[0].quantity} is not available for ${quantityAvaible[0].id}`)
    }
